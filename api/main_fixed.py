                ...
                if "/user/" in current_url or "/me" in current_url:
                    ...
                    user_info = await crawler._page.evaluate("""
                        () => {
                            ...
                            // 检查URL中是否有真实用户ID（登录后 /user/self 会重定向到 /user/用户ID）
                            const urlPath = window.location.pathname;
                            const userIdMatch = urlPath.match(/\/user\/([A-Za-z0-9_]+)/);
                            if (userIdMatch && userIdMatch[1] !== 'self') {
                                result.userId = userIdMatch[1];
                                result.debug.foundElements.push({type: 'userId', value: userIdMatch[1]});
                            }
                            
                            // 最终判断：必须有昵称或用户ID才算真正登录
                            result.hasUserInfo = !!(result.nickname || result.userId);
...
@app.post('/api/shipinhao/browser-extract')
async def shipinhao_browser_extract(request: ShipinhaoExtractRequest):
    """
    使用浏览器自动化提取视频号视频信息
    ...
    try:
        # 检查是否支持浏览器自动化
        try:
            from shipinhao_browser_extractor import get_extractor, close_extractor
        except ImportError:
            return JSONResponse(
                status_code=400,
                content={'detail': '浏览器自动化功能未安装，请安装 playwright'}
            )
        ...
        # 提取视频信息
        result = await extractor.extract_video(request.url)
        ...
        if result.get('status') in ['success', 'partial']:
            ...
            try:
                video_info = {
                    ...
                    'fans_count': result.get('fans_count', ''),
                    'liked_count': result.get('liked_count', ''),
                }
                
                # 保存到数据库
                await save_video_to_db(video_info)
        ...
        logger.error(f'浏览器提取失败: {str(e)}', exc_info=True)
        return JSONResponse(
            status_code=500,
            content={'detail': f'浏览器提取失败: {str(e)}'}
        )


async def save_video_to_db(video_info: dict):
    """保存视频信息到数据库"""
    from datetime import datetime
    ...
    video_data = {
        ...
        'collects': video_info.get('collects', 0),
        'fans_count': video_info.get('fans_count', ''),
        'liked_count': video_info.get('liked_count', ''),
        'created_at': datetime.now().isoformat(),
    }
    
    # 保存到数据库的逻辑（这里简化处理）
    # 在实际应用中，这里会连接数据库并插入数据
    print(f"保存视频到数据库: {video_data['video_id']}")
    return True
