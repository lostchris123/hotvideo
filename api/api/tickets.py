    ...
    def get_user_tickets(user_id: int, limit: int = 50, offset: int = 0) -> List[dict]:
        ...
        try:
            ...
            for t in tickets:
                ...
                if t.payment_proof:
                    # 返回相对路径，前端会自动加 /uploads/ 前缀
                    proof_path = t.payment_proof
                    # 去掉开头的 uploads/，只保留 payment_proofs/xxx.jpg
                    if proof_path.startswith('uploads/'):
                        payment_proof_url = proof_path[8:]  # 去掉 'uploads/'
                    elif proof_path.startswith('/uploads/'):
                        payment_proof_url = proof_path[9:]  # 去掉 '/uploads/'
                    else:
