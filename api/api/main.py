...
import sys
import re
from fastapi import FastAPI, HTTPException, Query, Depends, UploadFile, File, Form, status, Request
...
class SearchRequest(BaseModel):
    ...
    content_type: str = Field("video", description="内容类型: video/image，抖音和小红书有效")
...
async def open_login_page(platform: str = "douyin",
    ...
    """打开登录页面"""
    # 清理浏览器锁文件，防止"浏览器被占用"错误
    import os
    import glob
    lock_patterns = [
        f"/app/data/browser_profiles/user_{current_user.id}/*/SingletonLock",
        f"/app/data/browser_profiles/user_{current_user.id}/*/SingletonCookie",
        f"/app/data/browser_profiles/user_{current_user.id}/*/SingletonSocket",
    ]
...
async def cleanup_all_sessions(current_user: User = Depends(get_current_user)):
    ...
    # 清理浏览器锁文件
    lock_patterns = [
        f"/app/data/browser_profiles/user_{user_id}/*/SingletonLock",
        f"/app/data/browser_profiles/user_{user_id}/*/SingletonCookie",
        f"/app/data/browser_profiles/user_{user_id}/*/SingletonSocket",
    ]
    cleaned_locks = 0
    for pattern in lock_patterns:
        for lock_file in glob.glob(pattern):
    ...
    return {
        "success": True,
        "message": f"用户 {user_id} 的会话已清理，清理了 {cleaned_locks} 个锁文件"
    }
...
async def cleanup_all_sessions_admin(current_user: User = Depends(get_current_admin)):
    ...
    # 清理所有浏览器锁文件
    lock_patterns = [
        "/app/data/browser_profiles/*/SingletonLock",
        "/app/data/browser_profiles/*/SingletonCookie",
        "/app/data/browser_profiles/*/SingletonSocket",
        "/app/data/browser_profiles/*/*/SingletonLock",
        "/app/data/browser_profiles/*/*/SingletonCookie",
        "/app/data/browser_profiles/*/*/SingletonSocket",
    ]
    ...
    return {
        "success": True,
        "message": f"所有用户的会话已清理，清理了 {cleaned_locks} 个锁文件"
    }
...
async def open_login_in_session(
    ...
    # 检查是否需要在现有浏览器中打开新标签页
    existing_platform = session.get("platform")
    if existing_platform and existing_platform != platform:
        # 平台不同，在现有浏览器中打开新标签页
        await unified_session_manager._vnc_manager._open_url_in_browser(session, platform)
        logger.info(f"在现有浏览器中打开新标签页: {existing_platform} -> {platform}")
...
async def create_async_search(
    ...
    task = task_manager.create_task(
        ...
        platform=request.platform,
        content_type=request.limit,
        limit=request.limit,
        min_likes=request.min_likes
    )
    
    # 不再每次搜索都关闭浏览器，保持登录状态
    # 浏览器会话会被复用，在现有窗口中打开新标签页搜索
...
async def cancel_search_task(
    ...
    success = task_manager.cancel_task(task_id, current_user.id)
    if success:
...
async def search_videos(
    ...
    # 抖音/小红书特殊处理：视频和图文共用同一个爬虫，但数据存储在不同的表
    platform = request.platform
    if platform == "xiaohongshu" and request.content_type == "image":
        platform = "xiaohongshu_image"
    elif platform == "douyin" and request.content_type == "image":
        platform = "douyin_image"
    ...
    try:
        ...
        # 传递content_type给爬虫（抖音和小红书都支持图文搜索）
        if request.platform in ["xiaohongshu", "douyin"]:
