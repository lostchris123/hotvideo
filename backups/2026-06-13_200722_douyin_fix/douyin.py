...
class VideoInfo:
    ...
    is_video: bool = True  # 是否为视频（False表示图文）
    images: List[str] = field(default_factory=list)  # 图片URL列表（图文用）
...
class DouyinCrawler:
    ...
    def extract_video_duration_from_file(self, file_path: str) -> float:
        """
        使用 ffprobe 从本地视频文件提取时长
    ...
    async def search_by_keyword(
        ...
        Args:
            ...
            content_type: 内容类型: video(视频)或image(图文)
        ...
        logger.info(f"搜索关键词: {keyword}, 目标数量: {limit}, 最小点赞数: {min_likes}, 排序: {sort_by}, 内容类型: {content_type}")
        ...
        # type=video -> 视频搜索
        # type=image -> 图文搜索
        content_type_param = "general" if content_type == "image" else "video"
        
        # 构建搜索URL
        if sort_type is not None:
            search_url = f"{self.BASE_URL}/search/{keyword}?type={content_type_param}&sort_type={sort_type}"
            logger.info(f"使用排序参数 sort_type={sort_type}, 内容类型: {content_type_param}")
        else:
            search_url = f"{self.BASE_URL}/search/{keyword}?type={content_type_param}"
        ...
        for selector in selectors_to_try:
            try:
                ...
                logger.info(f"搜索结果已加载，选择器: {selector}")
                found = True
                break
            except:
                continue
                ...
                try:
                    ...
                    try:
                        video_info = await asyncio.wait_for(
                            self.get_video_detail(link['video_url'], min_likes=min_likes, timeout=timeout, cancel_check=is_cancelled),
                            timeout=timeout + 30
                        )
                    ...
                    if video_info and video_info.video_id:
                        if video_info.likes >= min_likes:
                            video_info.platform = "douyin_image" if not video_info.is_video else "douyin"
                            
                            # 根据内容类型过滤
                            if content_type == "image" and video_info.is_video:
                                logger.info(f"跳过视频内容: {video_info.video_id} (搜索图文模式)")
                                continue
                            elif content_type == "video" and not video_info.is_video:
                                logger.info(f"跳过图文内容: {video_info.video_id} (搜索视频模式)")
                                continue
                            ...
                            logger.info(f"符合条件并已保存 [{len(qualified_videos)}/{limit}]: 点赞={video_info.likes}, 作者={video_info.author_name}, 类型={'图文' if not video_info.is_video else '视频'}")
                            
                            percentage = min(100, round((len(qualified_videos) / limit) * 100)) if limit > 0 else 0
                            report_progress(
                                "visiting", 
                                len(visited_ids), 
                                detail_visits, 
                                len(qualified_videos),
                                f"找到符合条件的内容 [{len(qualified_videos)}/{limit}]，{percentage}%"
                            )
                        else:
        ...
        try:
            ...
            if video_info.video_stream_url:
                ...
                if video_path and video_path.exists():
                    ...
                    # 提取视频时长
                    try:
                        video_info.duration = self.extract_video_duration_from_file(str(video_path))
    ...
    async def save_single_video_to_db(self, video_info, keyword: str):
        """保存单个视频/图文到数据库"""
        from crawler.models import get_platform_session, DouyinVideoRecord, DouyinImageRecord
        import json
        
        # 根据is_video字段决定保存到哪个表
        if video_info.is_video:
            session = get_platform_session("douyin")
            ModelClass = DouyinVideoRecord
        else:
            session = get_platform_session("douyin_image")
            ModelClass = DouyinImageRecord
        ...
        try:
            ...
            if existing:
                ...
                # 图文特有字段
                if not video_info.is_video and hasattr(existing, 'images') and video_info.images:
                ...
                # 图文特有字段
                if not video_info.is_video and video_info.images:
    ...
    async def _extract_search_results_v2(self) -> List[VideoInfo]:
        ...
        if self.content_type == "image":
            # 图文搜索使用不同的选择器
            extractors = [
                self._extract_image_search_results,
    ...
    async def _extract_image_search_results(self) -> List[VideoInfo]:
        """提取图文搜索结果"""
        videos = []
        ...
        for selector in selectors:
            try:
                ...
                if elements:
                    logger.info(f"[图文搜索] 使用选择器 {selector} 找到 {len(elements)} 个元素")
                    for el in elements[:self.target_count * 2]:
                        try:
                            video_info = await self._extract_note_info(el)
                            if video_info and video_info.video_id:
    ...
    async def _extract_note_info(self, element) -> Optional[VideoInfo]:
        """从图文卡片提取信息"""
        try:
            # 获取链接
            link = await element.query_selector('a[href*="/note/"]')
            if not link:
                return None
            ...
            title = ""
            try:
                title_el = await element.query_selector('[class*="title"], h3, h4')
                if title_el:
            ...
            return VideoInfo(
                video_id=note_id,
                title=title,
                video_url=f"https://www.douyin.com/note/{note_id}",
                is_video=False
            )
        except Exception as e:
            return None
    ...
    async def get_video_detail(self, video_url: str, timeout: int = 60, min_likes: int = 0, cancel_check: Callable[[], bool] = None) -> VideoInfo:
        """
        ...
        if is_cancelled():
            logger.info(f"任务已取消，跳过视频详情: {video_id}")
            return None
        ...
        try:
            ...
            logger.info(f"点赞数 {basic_data.get('likes')} >= {min_likes}，等待内容元素加载...")
            ...
            try:
                ...
                if is_cancelled():
                    logger.info(f"任务已取消，停止提取视频流: {video_id}")
                    return None
                ...
                logger.warning(f"视频元素加载失败: {e}，尝试检测图文内容...")
                
                try:
                    images = await self._extract_images_from_page()
                    if images:
                        is_video = False
                        logger.info(f"检测到图文内容，共 {len(images)} 张图片")
                    ...
                    logger.warning(f"图文内容提取失败: {img_e}")
                    video_data = await self._extract_video_stream()
                    video_stream_url = video_data.get('video_stream_url', '')
                    audio_url = video_data.get('audio_url', '')
                    thumbnail_url = video_data.get('thumbnail_url', '')
    ...
    async def _extract_images_from_page(self) -> list:
        ...
        try:
            images = await self._page.evaluate("""
                () => {
                    ...
                    try {
                        ...
                        for (const script of scripts) {
                            ...
                            if (text.includes('__RENDER_DATA__')) {
                                ...
                                if (match) {
                                    ...
                                    // 尝试多种数据结构获取图片列表
                                    let imageList = renderData?.app?.videoInfo?.item_list?.[0]?.images ||
                                                   renderData?.videoInfo?.item_list?.[0]?.images ||
                                                   renderData?.aweme_detail?.images ||
                                                   renderData?.data?.item_list?.[0]?.images ||
                                                   renderData?.item_list?.[0]?.images ||
                                                   [];
                        ...
                        console.error('从 __RENDER_DATA__ 提取图片失败', e);
                    }
                    
                    // 方法2: 从页面图片元素提取
                    if (images.length === 0) {
                        const imgElements = document.querySelectorAll('img[src*="douyinpic"], img[src*="p3.douyinpic"], img[src*="p9.douyinpic"]');
                        for (const img of imgElements) {
                            const src = img.src || img.dataset?.src || '';
                            if (src && !src.includes('avatar') && !src.includes('default') && !images.includes(src)) {
                                images.push(src);
                            }
                        }
                    }
                    ...
                    if (images.length === 0) {
                        ...
                        for (const el of elements) {
                            ...
                            if (match && match[1] && !match[1].includes('avatar') && !images.includes(match[1])) {
                                images.push(match[1]);
                            }
                        }
                    }
            ...
            logger.info(f"从页面提取到 {len(images or [])} 张图片")
            return images or []
        except Exception as e:
    ...
    async def _extract_video_stream(self) -> dict:
        """提取视频流地址"""
        # 检查取消状态
        if hasattr(self, '_current_cancel_check') and self._current_cancel_check():
            logger.info("取消检查：跳过视频流提取")
            return {}
